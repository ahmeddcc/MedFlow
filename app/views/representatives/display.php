...
<head>
    ...
    <title>شاشة انتظار المناديب - MedFlow</title>
    ...
    <style>
        ...
        * {
        ...
        .current-number {
            ...
            text-shadow: 0 0 100px rgba(255, 107, 107, 0.5);
        ...
        .waiting-number-item .turn {
            font-size: 1.5rem;
        }
        
        .waiting-number-item .company {
            font-size: 0.875rem;
            color: var(--text-muted);
        }
...
<body>
    <header class="display-header">
        <div class="logo">
            ...
            <span class="header-badge">المناديب</span>
        </div>
    ...
    <main class="display-main">
        ...
        <div class="current-section">
            ...
            <div class="current-number" id="currentNumber"><?= $currentCall['full_turn'] ?></div>
            <div class="current-company"><?= $currentCall['company_name'] ?></div>
            <div class="current-message">يرجى التوجه لغرفة الطبيب</div>
            <?php else: ?>
            <div class="no-visitors">
                ...
                <p style="font-size: 1.5rem">في انتظار المناديب</p>
            </div>
            <?php endif; ?>
        </div>
        ...
        <aside class="waiting-section">
            ...
            <div class="waiting-numbers">
                ...
                <div class="waiting-number-item <?= $item['status'] === 'called' ? 'called' : '' ?>">
                    <span class="turn"><?= $item['full_turn'] ?></span>
    ...
    <script>
        ...
        // التحديث التلقائي
        let lastTurn = '<?= $currentCall['full_turn'] ?? '' ?>';
        
        function checkForUpdates() {
            fetch('<?= url('rep-waiting/status') ?>', {
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
            .then(r => r.json())
            .then(data => {
                if (data.current_turn !== lastTurn && data.current_turn) {
                    lastTurn = data.current_turn;
                    playRepCallAudio(data.current_turn);
        ...
        function playQueue(queue) {
            ...
            audio.onended = () => setTimeout(() => playQueue(queue), 100);
            audio.onerror = () => playQueue(queue);
            audio.play().catch(() => playQueue(queue));
        }
    </script>
</body>
</html>
